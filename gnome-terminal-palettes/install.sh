...
set_profile_colors() {
  ...
  case $scheme in
    dark  )
      local bg_color_file=$dir/colors/dark_bg
      local fg_color_file=$dir/colors/dark_fg
      if [ "$newGnome" = "1" ]
        then local palette_color_file=$dir/colors/dark_palette_new
        else local palette_color_file=$dir/colors/dark_palette
      fi
    ;;

    light )
      local bg_color_file=$dir/colors/light_bg
      local fg_color_file=$dir/colors/light_fg
      if [ "$newGnome" = "1" ]
        then local palette_color_file=$dir/colors/light_palette_new
        else local palette_color_file=$dir/colors/light_palette
      fi
    ;;
  esac
  ...
  if [ "$newGnome" = "1" ]
    ...
    # set color palette
    dconf write $profile_path/palette "[$(cat $palette_color_file)]"
    ...
    # set color palette
    gconftool-2 -s -t string $profile_path/palette $(cat $palette_color_file)
